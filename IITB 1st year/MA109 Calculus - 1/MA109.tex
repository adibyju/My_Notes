...
\definecolor{- }{RGB}{158,0,32} %250,0,32
...
\b{itemize}
    \i A {\color{lg}sequence} in a set $X$ is a function ${\color{lg}a}:\mathbb{N}\rightarrow X$, that is, a function from the natural numbers to $X$
...
\b{itemize}
    ...
    \i Two sequences $a_n$ and $b_n$ will be related to each other (and we write $a_n \sim b_n$) if:
    \b{equation*}
       \lim_{n~\rightarrow ~\infty} |a_n-b_n|=0
    \e{equation*}
    This is an equivalence relation and it is a fact that it partitions the set $S$ into disjoint classes. The set of disjoint classes is denoted by  $S/\hspace{-0.1cm}\sim$. If two sequences converge to the same limit, they are necessarily in the same class. A real number is an equivalence class in $S/\hspace{-0.1cm}\sim$. So a real number should be thought of as the collection of all rational sequences which converge to it.
    ...
    \b{itemize}
        \item[$-$] \tb{Theorem 1:} As $x \rightarrow x_0$, if $f (x) \rightarrow l_1$, $g(x) \rightarrow l_2$ and $h(x) \rightarrow l_3$ for functions $f , ~g, ~h$ on some interval $(a, b)$ such that $f (x) \leq g(x) \leq h(x)$ for all $x \in (a, b)$, then:
        \b{equation*}
        l_1\leq l_2\leq l_3 
        \e{equation*}
        \item[$-$] \tb{Theorem 2:} Suppose $\lim_{x~\rightarrow~x_0} f(x)=\lim_{x~\rightarrow~x_0} h(x)=l$ and If $g(x)$ is a function satisfying $f (x) \leq g(x) \leq h(x)$ for all $x \in (a, b)$, then $g(x)$ converges to a limit as $x\rightarrow x_0$ and: 
        \b{equation*}
...
\b{itemize}
    ...
    \i  Functions that satisfy the following property for some $\alpha$ (not necessarily greater than 1) are said to be {\color{lg}Lipschitz continuous} with exponent $\alpha$:
    \b{equation*}
    ...
    \i \tb{Def:} let $f : X \rightarrow\mathbb{R}$ be a function and $x_0$ be in $X$. Suppose there is a sub-interval $x_0 \in (c, d) \subset X$ such that $f (x_0) \geq f (x)$ (resp. $f (x_0) \leq f (x)$) for all $x \in (c, d)$, then $f$ is said to have a local maximum (resp. local minimum) at $x_0$ 
\e{itemize}
...
\b{itemize}
    ...
    \i \tb{Theorem:} if $f$ satisfies the hypotheses of the MVT, and further $f'(x_0)=0$ for every $x \in (a, b)$, then $f$ is a constant function
    ...
    \b{equation*}
       P_n(x)=\sum_{k~=~0}^{n}\frac{f^{k}(a)}{k!}(x-a)^k, ~ and~ R_n(x)=\frac{f^{n+1}(c)}{(n+1)!}(x-a)^{n+1}
    \e{equation*}
...
\b{itemize}
    ...
    \i \tb{Def:} Given a partition $P=\{a=x_0<x_1<\ldots <x_{n-1}<x_n=b\}$ and a function $f : [a, b] \rightarrow\mathbb{R}$, we define two associated quantities. First we set: $M_i=\sup_{x\hspace{0.05cm}\in\hspace{0.05cm}[x_{i-1},x_i]} f(x)$ and $m_i=\inf_{x\hspace{0.05cm}\in\hspace{0.05cm}[x_{i-1},x_i]} f(x), ~1\leq i\leq n$. We define the {\color{lg}Lower sum} as:
    \b{equation*}
