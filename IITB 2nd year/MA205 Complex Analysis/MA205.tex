...
\definecolor{green}{RGB}{60,220,189} %132,242,214
...
\b{itemize}
    ...
    \i {\color{green}Complex numbers ($\mathbb{C}$)} consists of all the numbers of the form $a + i b$ where $a$ and $b$ are in $\mathbb{R}$. $a$ is called the real part and $b$ is called the imaginary part.
...
\b{itemize}
    \i Let $\Omega \subseteq \mathbb{C}$ be a subset. We say that $\Omega$ is an {\color{green}open subset} of $\mathbb{C}$ if given any point $z_0 \in \Omega$, there exists $\delta > 0$ such that the set $\{z \in \mathbb{C}~\text{such that} ~|z-z_0| < \delta \} \subset \Omega$. Intuitively this means that if some point belongs to $\Omega$, all sufficiently nearby points belong to $\Omega$.
    \i $z$ is in the {\color{green}$\delta$-neighbourhood} {\color{green}($B_\delta (z_0)$)} of $z_0$ means that $z$ is inside the circle with center $z_0$ and radius $\delta$
    \i A subset $Z \subseteq \mathbb{C}$ is said to be {\color{green}closed} if its complement is open
    ...
    \e{itemize}
    \i We define {\color{green}closure} of a subset $S \subseteq \mathbb{C}$ to be the smallest closed set containing $S$. It is denoted $\overline{S}$. Equivalently, the closure of $S$ is the union of $S$ together with its limit points.
    \i A subset $S \subseteq \mathbb{C}$ is said to be {\color{green}path-connected} if given any 2 points $z_0,~z_1 \in S$, there exists a continuous path joining them
    \i An open subset of $\mathbb{C}$ which is path-connected is called a {\color{green}domain}
    ...
    \b{equation*}
       \lim_{z \hspace{0.05cm} \to\hspace{0.05cm} z_0} f(x) = L
    \e{equation*}
    \i A function $f:\Omega \subset \mathbb{C} \rightarrow{} \mathbb{C}$ is continuous at $z_0 \in \Omega$ if
    \b{equation*}
        f(z) = f(z_0)
    \e{equation*}
    \i The function $f$ is continuous on a domain if it is {\color{green}continuous} at every point in the domain
    \i A subset $S \subseteq \mathbb{C}$ is said to be {\color{green}compact} if it is closed and bounded
    ...
    \i \tb{differentiable} - let $\Omega \subset \mathbb{C}$ be open. A function $f:\Omega \subset \mathbb{C} \rightarrow{} \mathbb{C}$ is said to be differentiable, (sometimes called {\color{green}complex-differentiable}) at $z_0$ if the following condition exists:
    \b{equation*}
       f'(z_0) = \lim_{z\hspace{0.05cm} \to\hspace{0.05cm} z_0} \frac{f(z)-f(z_0}{z-z_0}
    \e{equation*}
    \i We say that $f$ is {\color{green}holomorphic} on $\Omega$ if $f$ is differentiable at each point of $\Omega$. $f$ is holomorphic (also called {\color{green}complex analytic}) at $z_0$ if it is holomorphic in some neighbourhood of $z_0$.
...
\b{itemize}
    ...
    $u_x,~u_y,~v_x,~v_y$ exist at $(a,b)$ and they satisfy {\color{green}$u_x=v_y$} and {\color{green}$u_y=-v_x$.} If CR equations are not satisfied at a point, then f is not differentiable at that point.
    \i \tb{Def:}
    \b{equation*}
      \pdv{}{z} = \frac{1}{2}\bigg(\pdv{}{x}-i\pdv{}{y}\bigg) \hspace{3cm}  \pdv{}{\overline{z}} = \frac{1}{2}\bigg(\pdv{}{x}+i\pdv{}{y}\bigg)
    \e{equation*}
...
\b{itemize}
    \i A real valued function $u:U \subset \mathbb{R}^2 \rightarrow{} \mathbb{R}$ is called {\color{green}harmonic} if it is twice continuously differentiable and satisfies $u_{xx} + u_{yy} = 0$ on $U$
    \i If $f = u + iv$ is holomorphic on $\Omega$, then both $u$ and $v$ are harmonic on $\Omega$
    \i Suppose $u$ and $v$ are harmonic functions on $\Omega$. We say that $v$ is a {\color{green}harmonic conjugate} of $u$ if $f=u+iv$ is holomorphic in $\Omega$. Note that $v$ is a harmonic conjugate of $u$ does not mean that $u$ is a harmonic conjugate of $v$.
    \i  If u and v are harmonic conjugates of each other they are constant functions
    \i A domain $\Omega \subseteq \mathbb{C}$ is said to be {\color{green}simply-connected} if it is path-connected and every path between two points can be continuously transformed into any other such path while preserving the two endpoints. Another definition is that the above domain is said to be simply connected if every simple closed curve in $\Omega$ has all its interior points belonging to $\Omega$. If $\Omega$ is simply-connected, then every simple closed curve can be continuously deformed to a point through a continuous family of closed curves.
    ...
    \e{equation*}
    In particular, $u$ does not attain its maximum at any interior point unless it is a constant.
...
\b{itemize}
    ...
    or more generally,
    \b{equation*}
        \sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}0}^{\infty} a_\mathrm{i}(z-z_0)^\mathrm{i}
    \e{equation*}
    \i A series of the form $\sum_{n\hspace{0.05cm}=\hspace{0.05cm}0}^{\infty} a_n$ of complex numbers is said to converge if the sequence of partial sums $s_n= \sum_{k\hspace{0.05cm}=\hspace{0.05cm}0}^{n} a_k$ converges (to a finite complex number)
    \i {\color{green}Radius of convergence} for a power series is a real number $R$ such that $\sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}0}^{\infty} a_\mathrm{i}(z-z_0)^\mathrm{i}$ converges when $|z-z_0| < R$, and diverges when $|z-z_0| > R$. In other words, the radius of convergence is the largest $R$ such that the given power series converges inside a disc of radius $R$. The radius of convergence  exists for any power series.
    \i The series $\sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}1}^{\infty} a_\mathrm{i}$ is said to be {\color{green}absolutely convergent} if the series  $\sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}1}^{\infty} |a_\mathrm{i}|$ is convergent
    ...
    \i \tb{upper limit/limit supremum/limit superior (denoted by lim sup):} for a sequence of real numbers $x_1,~x_2,\ldots$, let $y_n$ be the supremum of the set $\{x_n,~x_{n+1},\ldots \}$. Then the sequence $y_1,~y_2,\ldots$ is a monotonically decreasing sequence which diverges to $\infty$ or has a finite limit. This is called the upper limit of the sequence $\{x_\mathrm{i}\}$. It can be $\infty$ or $-\infty$. If the limit of the sequence $\{x_n\}$ exists, then the upper limit coincides with the usual limit.
    ...
    \i \tb{Theorem (Cauchy’s root test):} for a series $\sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}1}^{\infty} a_\mathrm{i} $ of complex numbers, let $C=\limsup_{\mathrm{i} \hspace{0.05cm}\to\hspace{0.05cm} \infty } \sqrt[\mathrm{i}]{|a_\mathrm{i}|}$. Then the series converges absolutely if $C<1$ and it diverges if $C>1$. The test is indecisive for $C = 1$
    \i \tb{Theorem (Ratio test):} for a series $\sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}1}^{\infty} a_\mathrm{i} $, let $L=\limsup_{\mathrm{i}\hspace{0.05cm} \to\hspace{0.05cm} \infty } |\frac{a_{{\mathrm{i}}+1}}{a_\mathrm{i}}|$. Then, if $L<1$, the series converges absolutely. The series diverges if there exists N such that $|\frac{a_{{\mathrm{i}}+1}}{a_\mathrm{i}}|>1$ for $i\geq N$. $L>1$ doesn’t imply that the series diverges.
    \i \tb{Theorem (existence of radius of convergence):} for the power series $\sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}1}^{\infty} a_\mathrm{i}(z-z_0)^\mathrm{i} $, let $R=\frac{1}{\limsup_{\mathrm{i}\hspace{0.05cm} \to\hspace{0.05cm} \infty } \sqrt[\mathrm{i}]{|a_\mathrm{i}|}}$. Then the power series converges absolutely if $|z-z_0|<R$ and diverges if $|z-z_0|>R$.
    ...
    \b{equation*}
       \limsup_{\mathrm{i}\hspace{0.05cm} \to\hspace{0.05cm} \infty } \sqrt[\mathrm{i}]{|a_\mathrm{i}|} \leq \limsup_{\mathrm{i} \hspace{0.05cm}\to\hspace{0.05cm} \infty}  {\left\lvert\frac{a_{{\mathrm{i}}+1}}{a_\mathrm{i}}\right\rvert}
    \e{equation*}
...
\b{itemize}
    ...
    \i {\color{green}Principle branch} of the logarithm is defined as follows: let $\Omega \subset \mathbb{C}$ be the open subset defined by $\mathbb{C}$ minus the negative real line. For any $z \in \Omega, ~z =\{~ |z|e^{i \theta} : -\pi < \theta <\pi~\}=re^{i\theta}$, define $f(z)=\log (r) +i\theta = \log {(|z|)} + i\arg{z} $.
...
\b{itemize}
    ...
    \i If $\gamma(t)=x(t)+iy(t)$ is a parametrized curve, then we define the length of $\gamma$ to be:
    \b{equation*}
       \int_{a}^{b} |\gamma'(t)|dt = \int_{a}^{b} \sqrt{x'(t)^2+y'(t)^2}dt
    \e{equation*}
    In case $\gamma$ is not continuously differentiable, i.e., $x(t),~y(t)$ are not $C^1$ functions of t, we define the length of the curve as follows: choose $n$ points $P_1, \ldots,P_n$ on the curve. Consider the sum $\sum_{1}^{n-1} |P_{{\mathrm{i}}+1}-P_\mathrm{i}|$, where $|P_\mathrm{i}-P_{{\mathrm{i}}-1}|$ denotes the length of the straight line joining $P_\mathrm{i}$ with $P_{{\mathrm{i}}-1}$. As $n$ tends to infinity and the length of each of these line segments goes to zero, we define the length of the curve to be the limit (if it exists) $\lim_{n \hspace{0.05cm}\to\hspace{0.05cm} \infty} \sum_{i}^{n}|P_{{\mathrm{i}}+1}-P_\mathrm{i}|$. The curve is called {\color{green}rectifiable} if the length exists.
    \i We say a curve $\gamma (t)=x(t)+iy(t)$ is {\color{green}smooth} if $\gamma '(t) \neq 0$ for all t. Such a curve is also called {\color{green}regular parametrized curve.}
    \i A {\color{green}contour} is a curve consisting of a finite number of smooth curves joined end to end. It is said to be {\color{green}simple} if the parametrization map is one to one except possibly at the end-points. (Intuitively it means that the curve does not cross itself). It is said to be {\color{green}closed} if the initial and end-points are the same, i.e., $\gamma(a)=\gamma(b)$.
    \i \tb{Jordan curve theorem:} any simple closed curve in $\mathbb{R}^2$ seperates the plane into two connected components. The curve is the common boundary of both of them. Exactly one of the components is bounded. 
    ...
    \b{itemize}
        ...
        \item[$-$] $\int_{C_1\cup C_2 \cup \ldots\cup C_n}f(z)dz=\int_{C_1}f(z)dz + \ldots + \int_{C_n}f(z)dz$
    ...
    \b{equation*}
       f(z)=\sum_{n\hspace{0.05cm}=\hspace{0.05cm}0}^{\infty} a_n(z-z_0)^n
    \e{equation*}
    where each $a_n$ is given by:
    \b{equation*}
       a_n=\frac{1}{2\pi i}\int_{|w-z_0|\hspace{0.05cm}=\hspace{0.05cm}r}^{} \frac{f(w)}{(w-z_0)^{n+1}}dw
    \e{equation*}
    ...
    \b{equation*}
       \int_{\gamma} \frac{f(z)}{(z-z_1)^{a_1}(z-z_2)^{a_2}\ldots (z-z_n)^{a_n}} dz = \sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}1}^{n} \int_{C_\mathrm{i}} \frac{f(z)}{(z-z_1)^{a_1}(z-z_2)^{a_2}\ldots (z-z_n)^{a_n}} dz
    \e{equation*}
    ...
    \e{equation*}
    \i A function defined all over $\mathbb{C}$ is called {\color{green}entire} if it is holomorphic everywhere in $\mathbb{C}$ 
    \i \tb{Liouville's theorem:} a bounded above entire function is a constant. A non-constant entire function has to be unbounded. 
    \i A continuous function $f:\mathbb{C} \rightarrow{} \mathbb{C}$ is said to be {\color{green}proper} if $|f(z)|\rightarrow{}\infty$ as $|z|\rightarrow{}\infty$. Equivalently $f$ is proper if it is continuous and inverse image of a bounded set is bounded.
...
\b{itemize}
    ...
    \i Isolated singularities are of two types: {\color{green}removable} and {\color{green}non-removable} singularities. If an isolated singularity can be removed by defining the function by assigning a certain value at that point, we say that the singularity is removable. If not we say it is non-removable.
    \i \tb{Reimann's removable singularity theorem:} $z_0$ is a removable singularity of $f$\hspace{0.05cm} iff $\hspace{0.1cm}\lim_{z\hspace{0.05cm}\rightarrow{}\hspace{0.05cm}z_0} f(z)$ exists 
    \i A {\color{green}pole} is a point at which the function blows up from all directions. An isolated singularity $z_0$ is said to be a pole if $\lim_{z\hspace{0.05cm}\rightarrow{}\hspace{0.05cm}z_0} f(z) \rightarrow{ }\infty$.
    \i \tb{order of a pole} - if $z_0$ is a pole of $f$, then there exists an integer $m>0$ such that $f(z)=(z-z_0)^{-m}f_1(z)$ on a punctured neighbourhood of $z_0$, for some function $f_1$ which is holomorphic on the neighbourhood of $z_0$. The smallest such integer $m$ is called the order of the pole. If $m$ is 1, then $z_0$ is said to be {\color{green}simple pole.}
    \i A function $f(z)$ defined on an open set except at all the poles is called a {\color{green}meromorphic function.} That is, the only singularities are poles.
    \i  An isolated singularity that is neither a pole nor a removable singularity is called an {\color{green}essential singularity}
...
\b{itemize}
    ...
    \b{equation*}
       f(z)=\frac{1}{2\pi i}\int_{|w-z_0|\hspace{0.05cm}=\hspace{0.05cm}R} \frac{f(w)}{w-w_0}dw - \frac{1}{2\pi i}\int_{|w-z_0|\hspace{0.05cm}=\hspace{0.05cm}r} \frac{f(w)}{w-w_0}dw
    \e{equation*}
    \i \tb{Laurent series:} 
    \b{equation*}
       f(z)=\sum_{n\hspace{0.05cm}=\hspace{0.05cm}-\infty}^{\infty} a_n(z-z_0)^n
    \e{equation*}
    where each $a_n$ is given by:
    \b{equation*}
       a_n=\frac{1}{2\pi i}\int_{|w-z_0|\hspace{0.05cm}=\hspace{0.05cm}r_0}^{} \frac{f(w)}{(w-z_0)^{n+1}} dw
    \e{equation*}
    ...
    \e{itemize}
    \i The {\color{green}principal part} of the Laurent series is:
    \b{equation*}
       f(z)=\sum_{n\hspace{0.05cm}=\hspace{0.05cm}-\infty}^{-1} a_n(z-z_0)^n
    \e{equation*}
    ...
    \e{equation*}
    if you integrate a Laurent series, only $a_{-1}$ remains, other terms vanish. What remains is called a {\color{green}residue:}
    \b{equation*}
       a_{-1}=\text{Res}(f;z_0)
    \e{equation*}
    ...
    \b{equation*}
       \int_{\gamma} f(z)dz=2\pi i \sum_{\mathrm{i}\hspace{0.05cm}=\hspace{0.05cm}1}^{n}\text{Res}(f;z_0)
    \e{equation*}
...
\b{itemize}
    \i \tb{Jordan's lemma:} let $f$ be a continuous function defined on the semicircular contour $C_R=\{Re^{i\theta} \hspace{0.05cm}|\hspace{0.05cm} \theta \in [0,\pi]\}$ of the form $f(z)=e^{iaz}g(z)$, where $g(z)$ is a continuous function and with $a>0$. Then:
    \b{equation*}
       {\left\lvert\int_{C_R} f(z)dz\right\rvert} \leq \frac{\pi}{a} \max_{\theta\hspace{0.05cm} \in\hspace{0.05cm} [0,\pi]} |g(Re^{i\theta})|
    \e{equation*}
