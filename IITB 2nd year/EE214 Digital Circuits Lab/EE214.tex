...
\b{itemize}
    ...
    \i \tb{very high speed integrated circuit HDL (VHDL)} - is a hardware description language which uses the syntax of ADA
    \i Uses of HDL:
    \b{itemize}
    ...
    \i When you assign a value to a variable the assignment becomes effective immediately, however when you assign a value to a signal there is an associated delay for this assignment as it is there in a real circuit
    ...
    \i Any description invokes a library by first declaring it (For e.g. Library IEEE;\hspace{0.05cm})
    \i Objects in the library can then be incorporated in the design by a {\color{green}use} clause (For e.g. Use IEEE.std\_logic\_1164.all;\hspace{0.05cm})
    \i Various types of objects in VHDL are {\color{green}constants, variables, signals} and {\color{green}files}
    \i Declaration of objects include their object type as well as the data type of values that they can acquire (For e.g. signal Enable: BIT;\hspace{0.05cm})
    \begin{figure}[H]
    \centering
    ...
    \i Objects which  are declared to be of {\color{green}Physical type,} carry a value as well as a unit. These are used to represent physical quantities such as time, resistance and capacitance. Time is the only physical type, which is pre-defined in the language. We can define other physical types when required.
...
\b{itemize}
    \i {\color{green}Behavioural style} describes a design in terms of its behaviour, and not in terms of a netlist of components. We describe behaviour through ``if-then-else” type of constructs, loops, sequential and concurrent assignment statements. Statements like “if-then-else” are inherently sequential. These must therefore occur only inside sequential bodies like processes.
...
\tb{JTAG:}
\b{itemize}
    \i {\color{green}Joint Test Action Group}
...
\b{itemize}
    ...
    \b{itemize}
        \item[$-$] \tb{wait on:} it is a long form version of the sensitivity list. The process will fire only if there is a change in any of the items in the statement.
        \item[$-$] \tb{wait until:} this involves a boolean condition to be true. There is a boolean expression which is given with this statement, and when it evaluates to true, the process is fired. This is used largely to make a system sensitive to a single edge or a single level.
        \item[$-$] \tb{wait for:} this is used to make the system wait for a fixed time. The time has to be specified. This is used more in testbenches where an input to a system needs to be applied for a fixed time, until a new one can be applied to get a reliable output.
        \item[$-$] \tb{wait:} just tell the process to die and never fire again. Usually used after application of the last input, to formally kill the process. Every process has to compulsorily have atleast one of these wait statements else the compiler will throw an error (so that the process need not be running all the time, thereby loading the simulation software).
    \e{itemize}
